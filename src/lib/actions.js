...
export const sites = {
  create: async (data, preview = null) => {
    ...
    await directus.request(createItem('sites', data.site));
    ...
    await directus.request(createItem('pages', home_page));

    await Promise.all([
      directus.request(createItem('symbols', symbols)),
      directus.request(createItem('pages', root_pages))
    ]);

    // upload preview to supabase storage
    if (preview) {
      await directus.from('sites').upload(`${data.site.id}/preview.html`, preview);
    }
