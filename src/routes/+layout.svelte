<script>
  ...
  import client from '$lib/cmsprovider/client'

  export let data

  let { cmsprovider, session } = data
  $: ({ cmsprovider, session } = data)
  ...
  onMount(() => {
    if (!cmsprovider) return
    // const {
    //   data: { subscription },
    // } = cmsprovider.auth.onAuthStateChange((event, _session) => {
    //   if (_session?.expires_at !== session?.expires_at) {
    //     invalidate('cmsprovier:auth')
    //   }
    // })
